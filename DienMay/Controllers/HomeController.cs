...
namespace DienMay.Controllers
{
    public class HomeController : Controller
    {
        ...
        private readonly DienMayContext _context;

        public HomeController(ILogger<HomeController> logger, DienMayContext context)
        {
            _logger = logger;
            _context = context;
        }
        ...
        public async Task<IActionResult> Index()
        {
            ...
            ViewBag.Tivi = await _context.Products.Where(a => a.Status == 1 && a.CategoryId==1).OrderBy(x => x.DisplayOrder).Take(5).ToListAsync();
            ViewBag.TuLanh = await _context.Products.Where(a => a.Status == 1 && a.CategoryId == 2).OrderBy(x => x.DisplayOrder).Take(5).ToListAsync();
            ViewBag.MayGiat = await _context.Products.Where(a => a.Status == 1 && a.CategoryId == 3).OrderBy(x => x.DisplayOrder).Take(5).ToListAsync();
            //Ds chuyên mục
